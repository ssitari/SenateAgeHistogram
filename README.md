...
countable rather than approximate; the subtitle names the range and the count. A
...
**Dragging inside an existing selection pans it**, keeping the width — sliding
the same 27-Congress window from 1831–1883 to 1881–1933 walks the median from 50
to 57, which is that whole jump seen through a fixed-width lens. The one
exception is a selection covering the entire strip: there is no empty track left
to start a new selection from, and a full-width selection has nowhere to pan to
anyway, so there a drag begins a fresh selection instead. The cursor follows —
`move` when a drag will pan, `crosshair` when it will select.
